...
- **DRAM 차트(모바일)**: 세로 크기 약 30% 확대, 범례를 2컬럼×3줄로 정리(줄 간격 약 50% 축소).
- **단기 모델 비교 표(모바일)**: 왼쪽으로 치우쳐 있던 표를 카드 폭에 맞게 칼럼이 고르게 퍼지도록 수정.
- **거시경제 지표(모바일)**: 지표명·값 컬럼을 오른쪽 정렬로 이동, 설명 컬럼 비중 확대.
- **글로벌 이벤트(모바일)**: 위험/유형 칸 폭 축소(82px→62px), 모니터링 결과(제목)를 최대 2줄까지 표시.
