...
| S-001 §07 글로벌 이벤트 | **카테고리 다양화 + 10건 보장** (#8) — 사용자 정의 4 카테고리(물리적 충돌 / 기상이변 / 금융 위기 / 기타)로 LLM·휴리스틱 강제 분류. RSS 쿼리 32개 (전쟁·지진·환율·유가·금리 17건 추가). 라운드-로빈으로 카테고리 다양성 + 위험도 우선순위 정렬. UI에 **유형 칩(빨강/황색/청색/회색)** + 위험도 + 제목 + 지역 함께 표시 (3건→10건). diversify_events() 부족 시 placeholder 자동 채움 | collect_news_events.py + dashboard.jsx §07 events-row | `.events-type-*`, `.events-row` |
