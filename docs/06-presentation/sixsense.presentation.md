...
footer: 'KAIST CAIO 10기 6조 · v2.3.1 · 2026-09-24'
style: |
  section {
...
<span class="small">KAIST CAIO 10기 6조 · v2.3.1 (2026-09-24 — 예측 검증 도입 + 화면 정직화 + 수집기 복구)</span>
...
| 영역 | 결과 |
|---|---|
| 🎯 **자동 수집** | 매주 화 06:00 무료 자동 실행 (GitHub Actions) · 이번 주 14개 신호 중 **정상 11 · 갱신 중단 3** (화면에 사유 표시) |
| 🔍 **예측 검증** | AI 모델 오차 <span class="neg">33.3%</span> vs 단순 기준선(지난주 값 유지) <span class="pos">15.7%</span> → **불합격, 예측 미표시** |
...
4. **신선도 감시** — 수집이 멈추거나 같은 값이 너무 오래 이어지면(주간 8주·월간 13주·분기 26주) "갱신 중단" 자동 표시
...
| "수집 안정성 최근 8주 100% 성공" | 고정 문장 | 실제 상태: 정상 11 · 갱신 중단 3 |
...
- 판정 규칙: **수집 14일 이상 없음** 또는 **같은 값이 원천 주기보다 오래 이어짐**(주간 8주 · 월간 13주 · 분기 26주, 기준금리는 동결이 정상이라 제외) → "갱신 중단" + 사유
...
|---|---|---|---|
| 평균 오차 (MAPE) | <span class="neg">33.3%</span> | 21.2% | <span class="pos">15.7%</span> |
| 기준선을 이긴 비율 | 15.6% | 51.9% (p = 0.299, 우연과 구분 안 됨) | — |
| 오르내림 적중률 | 32.0% | 66.7% | "항상 오른다" 찍기 70.6% |
| 4주 대기 여부를 모델대로 정했을 때 구매 단가 | <span class="neg">+9.29%</span> | 0% (한 번도 기다리지 않음) | 0% (미래를 안다면 -4.9%) |
...
- 주의: 뉴스 감성 신호는 실행할 때마다 과거 값도 다시 계산되어(검색 결과·LLM 판단이 매번 다름) 검증 수치가 실행마다 조금씩 달라짐 — 판정(불합격)은 매번 같음
...
| 5 | **S-014 수집 현황** | 정상 11 · 갱신 중단 3 · 사유 |
...
| <span class="pill pill-p0">P0</span> | **멈춘 수집** | 갱신 중단 3개 (A-2 수집기 없음 · A-3/A-4 원천 정체) — 거시 6종 · A-1 · A-7 은 v2.3.1 에서 복구 | A-2 수집기 · 원천 갱신 주기 확인 |
...
| 예측 검증 | **불합격** — 모델 33.3% vs 단순 기준선 15.7% (231회 워크포워드) |
| 수집 현황 | 14개 신호 중 정상 11 · 갱신 중단 3 · 거시 6종 모두 정상 |
| AI 요약 | Gemini 무료 티어 · 사실만 · "검증되지 않은 해석" 표시 |
| 자동화 | GitHub Actions 매주 화 06:00 KST + 앱 버튼 (비용 0원) |
| GitHub commits | **107개** |
| 현재 버전 | **v2.3.1** (2026-09-24) |
