---
marp: true
theme: default
size: 16:9
paginate: true
header: 'Sixsense · 메모리 반도체 시장 신호 모니터링'
footer: 'KAIST CAIO 10기 6조 · v2.5 · 2026-09-26'
style: |
  section {
    font-family: 'Pretendard Variable', 'Pretendard', 'Apple SD Gothic Neo', -apple-system, sans-serif;
    font-size: 22px;
    background: #fafaf8;
    color: #1a1a1a;
  }
  h1 { color: #1a1a1a; font-size: 38px; border-bottom: 2px solid #1a1a1a; padding-bottom: 8px; }
  h2 { color: #4a4a48; font-size: 28px; }
  h3 { color: #4a4a48; font-size: 22px; }
  table { font-size: 18px; border-collapse: collapse; }
  th { background: #f4f3ef; padding: 6px 10px; border: 1px solid #d8d4cc; text-align: left; }
  td { padding: 6px 10px; border: 1px solid #e8e6e0; }
  strong { color: #1a1a1a; font-weight: 700; }
  code { background: #f4f3ef; padding: 2px 6px; border-radius: 3px; font-size: 18px; }
  .small { font-size: 16px; color: #4a4a48; }
  .muted { color: #8a8884; }
  .pos { color: #16a34a; font-weight: 700; }
  .neg { color: #dc2626; font-weight: 700; }
  .neu { color: #ca8a04; font-weight: 700; }
  .info { color: #2563eb; font-weight: 700; }
  .pill { display: inline-block; padding: 2px 10px; border-radius: 12px; font-size: 14px; font-weight: 600; }
  .pill-p0 { background: #fef2f2; color: #dc2626; border: 1px solid #dc2626; }
  .pill-p1 { background: #fefce8; color: #ca8a04; border: 1px solid #ca8a04; }
  .pill-p2 { background: #eff6ff; color: #2563eb; border: 1px solid #2563eb; }
  section.title {
    background: linear-gradient(135deg, #1a1a1a 0%, #4a4a48 100%);
    color: #fafaf8;
  }
  section.title h1 { color: #fafaf8; border-bottom: 2px solid #fafaf8; font-size: 56px; }
  section.title h2 { color: #c6c4be; }
  section.title .small, section.title .muted { color: #c6c4be; }
  section.demo { background: #1a1a1c; color: #f4f3ef; }
  section.demo h1 { color: #f4f3ef; border-bottom: 2px solid #f4f3ef; }
---

<!-- _class: title -->

# Sixsense

## 메모리 반도체 시장 신호 모니터링 대시보드

<br>

**21개 공개 신호를 매주 자동 수집·요약하고, AI 가격 예측은 정직하게 검증해 판정·이유와 함께 보여주는 대시보드**

<br><br>

<span class="small">KAIST CAIO 10기 6조 · v2.5 (2026-09-26 — 예측 검증 + 화면 정직화 + 수집기 복구 + 재현성 + 불합격 예측의 참고 표시 + 최종 검증 반영)</span>

---

# 목차

| # | 주제 | 시간 |
|---|---|---|
| 1 | **Executive Summary** — 한눈에 요약 | 3분 |
| 2 | **Why & What** — 페르소나·핵심가치 | 3분 |
| 3 | **화면 구성** — v2.3 정직화 | 3분 |
| 4 | **Architecture & Pipeline** — 데이터 흐름 5단계 | 3분 |
| 5 | **예측 검증** — 우리가 발견한 것 | 4분 |
| 6 | **라이브 데모** — https://sixsense-eta.vercel.app | 3분 |
| 7 | **발전 방향** — 신뢰할 수 있는 예측으로 가는 길 | 2분 |

<br>

<span class="small">총 **21분** + Q&A</span>

---

<!-- ────────────────────── 1. Executive Summary ────────────────────── -->

# 1. Executive Summary

## 한 줄 정의

> 메모리 반도체 시장의 **공개 신호 21종**(주가·수출·재고·스팟가·뉴스 감성·지정학·거시)을 매주 자동 수집하고, **Gemini 가 사실만 요약**하며, **AI 가격 예측은 워크포워드 백테스트로 검증**해 합격·불합격 판정과 그 이유를 예측값 옆에 함께 보여주는 모니터링 대시보드

## 4대 핵심 결과

| 영역 | 결과 |
|---|---|
| 🎯 **자동 수집** | 매주 화 06:00 무료 자동 실행 (GitHub Actions) · 이번 주 14개 신호 중 **정상 12 · 갱신 중단 1 · 제외 1(A-4 데이터 오류)** (화면에 사유 표시) |
| 🔍 **예측 검증** | AI 모델 오차 <span class="neg">33.8%</span> vs 단순 기준선(지난주 값 유지) <span class="pos">15.7%</span> → **불합격** — 예측값은 "❌ 불합격 · 참고용" 표시와 불합격 이유 설명을 붙여 표시 |
| 🤖 **AI 요약** | Gemini 무료 티어 · 예측·신뢰도 없이 **수집된 사실만** 요약 · 모델별 무료 한도 순회 |
| 📱 **iOS 앱 + 원격 갱신** | PWA 홈 화면 설치 · 앱 버튼 → GitHub Actions → 자동 재배포 (약 5분, 서버·비용 0원) |

---

# 1. Executive Summary — 한눈에

```
┌─────────────────────────────────────────────────────────────────────────┐
│  매주 화요일 06:00 KST  (또는 앱의 "🔄 수동 갱신" 버튼)                  │
│                                                                          │
│  ① auto_collectors.py    →  신호 수집 (Yahoo/SEC/FRED/KOSIS/관세청/AWS/ │
│                              Manifold/HN/GPR/RSS)                        │
│  ② collect_news_events   →  RSS → Gemini 분류 → 뉴스 10 + 이벤트 10     │
│  ③ honest_backtest.py    →  예측 검증 (워크포워드, 모델 vs 기준선)       │
│                              + 지금부터 1~7주 예측 (검증과 같은 모델)    │
│  ④ build_insight.py      →  Gemini 사실 요약 + 불합격 이유(숫자 대조)   │
│  ⑤ build_frontend_data   →  frontend/src/mocks/data.js                 │
│                                              ↓ 봇 커밋 → Vercel 재배포   │
│                              https://sixsense-eta.vercel.app             │
└─────────────────────────────────────────────────────────────────────────┘
```

**프론트엔드** React + Vite (정적 배포, Vercel) · **파이프라인** Python (GitHub Actions) · **LLM** Gemini 무료 티어

---

# 1. Executive Summary — 버전 이력

| 버전 | 날짜 | 핵심 |
|---|---|---|
| v1.0 | 2026-06-11 | Phase 7 최초 완성 (14화면 hand-off 포팅) |
| v2.0 / v2.1 | 2026-07-12 | iOS 전용앱(PWA) 전환 · 모바일 UX |
| v2.2 | 2026-07-17 | 수동 갱신 버튼 아이폰 실작동 (GitHub Actions 무료 파이프라인) |
| v2.3 | 2026-09-24 | 예측 검증 도입 · 가짜 표시 제거 · 신선도 감시 · LLM Gemini 단독 |
| v2.3.1 | 2026-09-24 | 거시 6종 · A-1 · A-7 주간 수집 복구 · 신호 주기별 신선도 기준 |
| v2.3.2 | 2026-09-25 | 지나간 주 값 고정(재현성) · A-3 조회 기간 고정 해제(4월→8월) · 월간 통계 요일 무관 |
| v2.4 | 2026-09-25 | 불합격 예측도 참고용 표시 · "왜 불합격인가" Gemini 설명(숫자 자동 대조) |
| v2.4.1 | 2026-09-25 | A-4 제외 — 조회 주소가 다른 표를 가리켜 값이 재고지수가 아니었음 |
| **v2.5** | **2026-09-26** | **최종 검증 반영 — 오류 7 수정 · p값 겹침 보정 검정 · 발표 지연 7주 · 주간 작업 경량화(5분 30초→4분)** |

<br>

**Git history**: 127 commits · 문서: [prd.md](../../prd.md) · [plan](../01-plan/features/sixsense.plan.md) · [design](../02-design/features/sixsense.design.md) · [CHANGELOG](../../CHANGELOG.md)

<span class="small">※ plan·design·QA 등 PDCA 문서는 v2.2 이전에 작성되어 예측 성능 서술이 v2.3 검증 결과와 다릅니다. 최신 사실은 CHANGELOG v2.3 기준.</span>

---

<!-- ────────────────────── 2. Why & What ────────────────────── -->

# 2. Why — 왜 이 문제인가

## 서버 DRAM 가격을 읽기 어려운 이유

- **단일 신호로 불가**: 공급(팹·재고)·수요(AI CapEx)·거시(환율·금리)·지정학(대만·중국)이 **동시에** 작용
- **시장 데이터 비공개**: DRAMeXchange/TrendForce 계약가는 유료 → 우리는 **메모리 3사 주가지수를 대용 지표**로 사용
- **언어 장벽**: 핵심 정보가 대만(중문)·한국(한글)·미국(영문)에 분산
- **주기 미스매치**: 분기 IR ↔ 월간 통계 ↔ 주간 의사결정 ↔ 일간 거시

## Sixsense의 해법

**흩어진 공개 신호를 한 화면에 · 매주 자동으로 · 출처와 기준일을 붙여서 — 그리고 예측은 검증 판정과 함께**

---

# 2. What — 3 페르소나 × 1 화면

| 페르소나 | 핵심 니즈 | 사용 흐름 |
|---|---|---|
| **P1 메모리 기획팀장** | 주간 회의용 시장 상황 파악 | S-001 메인 → 지표 스냅샷 + 신호 요약 + 이벤트 |
| **P2 시장정보 애널리스트** | 신호의 원자료·출처·신선도 확인 | S-003/S-004 신호 상세 (실측 26주) → S-014 수집 현황 |
| **P3 영업/조달 담당** | 거시 환경과 글로벌 이벤트 | S-008 거시지표 + S-010 이벤트 + S-007 뉴스 원문 |

## 5대 핵심 가치

1. **자동 수집** — 매주 화 06:00 KST 무인 갱신, 비용 0원
2. **정직한 검증** — 예측은 워크포워드 백테스트로 단순 기준선과 비교, 합격·불합격과 그 이유를 예측값 옆에 표시
3. **출처·기준일 표시** — 모든 수치에 수집 출처와 데이터 날짜
4. **신선도 감시** — 수집이 멈추거나 같은 값이 너무 오래 이어지면(주간 8주·월간 17주·분기 26주) "갱신 중단" 자동 표시
5. **사실 기반 AI 요약** — 입력된 수치만 사용, 예측·신뢰도 생성 금지

---

<!-- ────────────────────── 3. 화면 구성 ────────────────────── -->

# 3. v2.3 화면 정직화 — 무엇을 없앴나

> 원칙: **측정·수집된 값만 표시. 모든 숫자는 출처와 기준일이 있어야 한다. 고정 숫자·예시 문장 금지.**

| 이전 표시 | 실제 | v2.3 |
|---|---|---|
| "현재 계약가 · DDR5 8Gb · $7.47/GB" | 메모리 3사 주가지수 ÷ 100 | **메모리 3사 주가지수 751.1 pt** + 대용 지표 고지 |
| "1~7주 AI 예측가" | 이미 지난 7주의 복기값 | 삭제 → 실측 4주 변화 |
| "8~21주 예측가 · 신뢰 81%/74%" | LSTM 실제 예측은 약 -60%, 신뢰도는 코드에 고정 | 삭제 → 예측 검증 결과 |
| "CLAUDE 종합 판단 · 신뢰 90%" | 실제는 Gemini, 신뢰도는 LLM이 적은 숫자 | "AI 요약 · Gemini · 검증되지 않은 해석" |
| 신호 상세 "원본 데이터" 표 | `Math.random()` 무작위 숫자 | 실측 26주 이력 |
| "오차 원인 분석: 재학습 완료, 6.0%→3.5%" | 한 적 없는 작업 (고정 문장) | 삭제 |
| "수집 안정성 최근 8주 100% 성공" | 고정 문장 | 실제 상태: 정상 12 · 갱신 중단 1 · 제외 1 |

---

# 3. 화면 맵 (v2.3)

| ID | 형태 | 이름 |
|---|---|---|
| S-001 | Full | **메인 대시보드** — 지표 스냅샷 · 신호 요약 · 추이 · 신호 · 뉴스 · 거시 · 이벤트 · 검증 · 수집 |
| S-003 | Modal | 정형 신호 상세 (실측 26주 이력 · 출처 · 신선도) |
| S-004 | Modal | 비정형 신호 상세 (뉴스 감성 · 해커뉴스 지표) |
| S-006 | Full | AI 뉴스 전체 목록 |
| S-007 | Modal | 뉴스 원문 링크 & AI 요약 |
| S-008 | Full | 거시경제 6탭 (실제 출처 · 기준일) |
| S-009 | Modal | 8주 전 vs 지금 (실측 비교) |
| S-010 / S-011 | Full / Modal | 글로벌 이벤트 목록 / 상세 |
| S-012 | Full | **AI 가격 예측 검증** (워크포워드 백테스트 상세) |
| S-014 | Full | 데이터 수집 현황 (수집일 · 갱신 중단 사유) |

<span class="small">삭제: S-002 예측 근거(고정 기여도) · S-005 Graph RAG(고정 상관계수 +0.72) · S-013 당시 신호(고정 사후분석)</span>

---

<!-- ────────────────────── 4. Architecture & Pipeline ────────────────────── -->

# 4. Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│  iPhone (PWA) / Browser — https://sixsense-eta.vercel.app        │
│  React 19 + Vite · src/mocks/data.js ← AUTO-GENERATED            │
│         │ "🔄 수동 갱신"                                          │
│         ▼                                                         │
│  Vercel 서버리스 함수 /api/refresh → GitHub workflow_dispatch     │
└─────────┼─────────────────────────────────────────────────────────┘
          ▼
┌───────────────────────────────────────────────────────────────────┐
│  GitHub Actions (매주 화 06:00 KST · 무료)                          │
│    ① auto_collectors      → data/historical/*.json                 │
│    ② collect_news_events  → data/news + events                     │
│    ③ honest_backtest      → data/validation/latest.json            │
│    ④ build_insight        → data/insight/latest.json               │
│    ⑤ build_frontend_data  → frontend/src/mocks/data.js            │
│    → 봇 커밋 → Vercel 자동 재배포                                   │
│  External: Yahoo·SEC·FRED·KOSIS·관세청·AWS·Manifold·HN·GPR·RSS    │
│  LLM: Gemini 무료 티어 (모델별 한도 순회)                           │
└───────────────────────────────────────────────────────────────────┘
```

---

# 4. 신호 매핑 (실제 수집 방식 그대로)

| Group | ID | 이름 | 실제 소스 |
|---|---|---|---|
| **정형 A** | A-1 | 대만 파운드리 주가 | Yahoo TSMC 70% + UMC 30% (각각 기준일=100 정규화) |
| | A-3 | 관세청 메모리 수출 | data.go.kr HS 854232 월간 |
| | A-4 | 전자부품 재고지수 | ⛔ 제외 — 조회 주소 오류 (아래 정정) |
| | A-5 | AWS 스팟 가격 | EC2 m6i.xlarge 스팟 (최근 90일) |
| | A-6 | 대만 침공 예측시장 | Manifold Markets |
| | A-7 | 구리 선물가 | Yahoo HG=F |
| **비정형 B** | B-1 · B-5 · B-6 | 실적발표 · LTA · HBM 뉴스 감성 | 구글 뉴스 헤드라인 → Gemini 감성 점수 |
| | B-7 | HN 메모리 화제도 | Hacker News 게시글 점수 |
| **거시** | 6종 | 10년물·기준금리·DXY·산업생산·USD/KRW·구리 | FRED + Yahoo |
| **대상** | target | 메모리 3사 주가지수 | MU 50% + SK하이닉스 30% + 삼성 20%, 2025-06-16=100 |

<span class="small">화면 미표시(수집·검증에는 사용): A-2 빅테크 CapEx · B-2 대만 뉴스 · B-3 HN · B-4 지정학(GPR)</span>

---

# 4. 신선도 — 수집이 된다고 데이터가 새것은 아니다

| 신호 | 상태 | 사유 (자동 판정) |
|---|---|---|
| A-2 빅테크 CapEx | ⚠ 갱신 중단 | 2026-07-12 이후 수집기 없음 (분기 데이터) |
| A-3 관세청 수출 | ✅ 정상 | 2026-08 통계까지 반영 (v2.3.2 — 아래 정정 참고) |
| A-4 전자부품 재고지수 | ⛔ 제외 | 조회 주소가 다른 표(품목별 광공업 생산·출하량, 반도체 없음)를 가리킴 — 올바른 주소 등록 전까지 화면·예측에서 제외 |
| A-1 · A-5 · A-6 · A-7 · B-1 · B-5 · B-6 · B-7 · 거시 6종 · 대상 지수 | ✅ 정상 | 매주 새 값 (A-1 · A-7 · 거시 6종은 v2.3.1 에서 수집 복구) |

<br>

- 판정 규칙: **수집 14일 이상 없음** 또는 **같은 값이 원천 주기보다 오래 이어짐**(주간 8주 · 월간 17주 · 분기 26주, 기준금리는 동결이 정상이라 제외) → "갱신 중단" + 사유
- 한국 정부 API(A-3·A-4)는 GitHub 서버(미국 IP)에서 차단된 적이 있음 → 실패하면 한국에서 로컬 수집으로 보충 (2026-09-24~25 미국 서버 4회 실행은 모두 정상)
- **정정 2**: v2.3.2 에서 A-4 를 "정상(발표 지연)"이라 했으나 사실이 아님 — 조회 주소가 반도체가 없는 다른 표를 가리켜, 달마다 330행 중 임의 한 행이 저장되고 있었음 (2026-05-17 도입 이후). v2.4.1 에서 화면·예측에서 제외
- **정정**: 이전 발표에서 A-3을 "원천 데이터 미갱신"이라 했으나 사실이 아님 — 우리 코드가 조회 기간을 2026-04 까지로 고정해 5~8월 발표분을 가져오지 못했음 (v2.3.2 수정, 8월 수출 157억 달러)
- AI 요약은 갱신 중단 신호를 근거로 쓰지 않음

---

<!-- ────────────────────── 5. 예측 검증 ────────────────────── -->

# 5. 예측 검증 — 방법 (워크포워드 백테스트)

> "그 시점에 실제로 알 수 있던 정보만으로, 매번 새로 학습해서 맞혔는가?"

- **대상**: 메모리 3사 주가지수 · 67주 (2025-06-16 ~ 2026-09-21)
- **방법**: 과거 27~39개 시점(예측 기간별)마다 그 시점까지 정답이 확정된 데이터로만 **새로 학습** → 1~7주 뒤 예측 (총 231회)
- **데이터 교정**: 날짜를 월요일로 통일 · 미래 값으로 과거를 채우지 않음 · 월간 통계는 **발표 지연 7주** 반영
- **비교 기준**: **단순 기준선** = "지난주 값이 그대로 유지된다"
- **합격 기준**: 모델 평균 오차 < 기준선 평균 오차 **그리고** 4주 예측의 Diebold–Mariano 검정(겹치는 예측 보정) p < 0.05
- 매주 자동 재실행 → 화면 §07 · S-012 에 표시

---

# 5. 예측 검증 — 결과: 불합격

| | 앱이 쓰던 방식 (가격 수준) | 개선 시도 (변화율) | 단순 기준선 |
|---|---|---|---|
| 평균 오차 (MAPE) | <span class="neg">33.8%</span> | 20.3% | <span class="pos">15.7%</span> |
| 기준선을 이긴 비율 | 15.2% | 53.2% (p = 0.686, 우연과 구분 안 됨) | — |
| 오르내림 적중률 | 32.0% | 68.4% | "항상 오른다" 찍기 70.6% |
| 4주 대기 여부를 모델대로 정했을 때 구매 단가 | <span class="neg">+9.31%</span> | 0% (한 번도 기다리지 않음) | 0% (미래를 안다면 -4.9%) |

<br>

- 앱 방식은 상승장에서 **33번 모두 "기다려라"** → 실제로 옳았던 건 7번
- 원인: 트리 모델은 학습 때 본 최고값 위를 예측하지 못함 (지수가 67주에 7.5배 상승)
- 근본 한계: **데이터 67주 · 실제 DRAM 가격이 아닌 주가 대용 지표**
- 화면 표시(v2.4): 불합격이어도 지금 1~7주 예측을 "참고용"으로 표시 — 차트 점선(개선 시도) + 기준선 점선, 예측 옆 불합격 판정·과거 오차·AI 불합격 이유
- p값 정정(v2.5): 예전 방식(1~7주 예측을 독립으로 세는 부호검정)은 겹치는 예측 때문에 p 가 작게 나왔음 — 개선 시도 0.146 → 겹침 보정 후 0.686
- 재현성: 지나간 주의 값은 마감 후 고정(v2.3.2) → **같은 데이터로 다시 돌리면 결과가 완전히 같음** (로컬·미국 서버 결과 파일 동일 확인). 수치는 새 주가 추가될 때만 바뀜

---

# 5. 이전 발표 수치는 왜 틀렸나

| 이전 주장 | 실제 원인 |
|---|---|
| 단기 MAPE **11.05%** (XGBoost ⭐) | 검증 구간 정답이 학습에 섞임 (**데이터 누수**) — 누수 제거 시 한때 0.03%까지 떨어져 드러남 |
| 중장기 MAPE **9.19%** (LSTM ⭐) | 실측값을 읽지 못하면 **코드에 박아둔 기본값**을 표시 |
| 단기 4.54% / 6.86% / Prophet 7.54% | 모델 이름 불일치로 파싱 실패 → **하드코딩 기본값** |
| "7주 예측" 차트 | 이미 지난 7주의 복기값을 오늘 가격에 이어 붙임 |
| 데이터 125행 | 요일 불일치로 **한 주가 두 줄로 분할** → "7칸 뒤"가 실제로는 약 3.5주 |

<br>

**교훈**: 검증 없는 정확도 수치는 발표하지 않는다. 합격 기준을 먼저 정하고, 예측을 보여줄 때는 판정과 과거 오차를 반드시 옆에 붙인다.

---

<!-- ────────────────────── 6. Live Demo ────────────────────── -->

<!-- _class: demo -->

# 6. 라이브 데모

## https://sixsense-eta.vercel.app (iPhone 홈 화면 앱)

<br>

| Step | 화면 | 강조 포인트 |
|---|---|---|
| 1 | **S-001 §01 지표 스냅샷** | 주가지수 751.1 pt · 대용 지표 고지 · 실측 4주 +14.4% · 예측 검증 "불합격" |
| 2 | **시장 신호 요약 카드** | Gemini 가 수집 사실만 요약 · "검증되지 않은 해석" 라벨 |
| 3 | **신호 카드 → S-003 A-3** | 실측 26주 · 출처·기준일 · 8월 수출 157억 달러 (조회 기간 고정 해제 후) |
| 4 | **§07 → S-012 예측 검증** | "왜 불합격인가" 설명 · 지금 1~7주 예측(참고용) · 기간별 오차 · 기준선 비교 |
| 5 | **S-014 수집 현황** | 정상 12 · 갱신 중단 1 (A-2) · 제외 1 (A-4) · 사유 자동 표시 |
| 6 | **"🔄 수동 갱신"** | GitHub Actions 무료 파이프라인 → 약 5분 후 자동 재배포 |

---

<!-- _class: demo -->

# 6. 예상 Q&A

**Q1. 불합격인데 왜 예측값을 보여주나요?**
→ 숨기면 모델이 무엇을 말하는지, 얼마나 틀리는지 판단할 수 없습니다. 그래서 예측값 옆에 항상 "❌ 불합격 · 참고용"과 과거 오차(예측 vs 기준선)를 붙이고, Gemini 가 쓴 불합격 이유를 보여줍니다. 설명 속 숫자는 검증 수치와 자동 대조해 하나라도 다르면 설명을 표시하지 않습니다. 예측값에는 상승·하락 화살표를 붙이지 않습니다.

**Q2. 가격 단위는?**
→ 실제 DRAM 계약가는 유료라, 메모리 3사 주가를 2025-06-16=100 으로 정규화한 **지수(pt)** 입니다. 계약가가 아님을 화면에 명시합니다.

**Q3. AI 요약은 믿을 수 있나요?**
→ Gemini 에 입력한 수치만 쓰도록 제한하고, 갱신 중단 신호는 근거에서 제외합니다. 그래도 LLM 문장이므로 "검증되지 않은 해석"으로 표시합니다.

**Q4. 자동화는?**
→ GitHub Actions 로 매주 화 06:00 KST 운영 중이며 비용 0원입니다. 앱 버튼으로도 실행됩니다.

---

<!-- ────────────────────── 7. 로드맵 ────────────────────── -->

# 7. 신뢰할 수 있는 예측으로 가는 길 — P0

| # | 항목 | 현재 | 필요 |
|---|---|---|---|
| <span class="pill pill-p0">P0</span> | **예측 대상** | 메모리 3사 주가지수 (대용) | 실제 DRAM 계약가 데이터 (유료 또는 TrendForce 공개 분기 변동률) |
| <span class="pill pill-p0">P0</span> | **데이터 길이** | 67주 | 여러 가격 사이클을 포함한 5년 이상 |
| <span class="pill pill-p0">P0</span> | **멈춘 수집** | 갱신 중단 1개 (A-2 수집기 없음) — 거시 6종 · A-1 · A-7 복구(v2.3.1), A-3 조회 기간 고정 해제(v2.3.2) | A-2 분기 CapEx 수집기 |
| <span class="pill pill-p0">P0</span> | **검증 관문** | 불합격이면 "참고용" 표시 + 이유 설명 (v2.4) | 실제 DRAM 가격 데이터로 재검증해 합격 수준 도달 |
| <span class="pill pill-p0">P0</span> | **수동 갱신 보호** | 누구나 호출 가능 | 인증 토큰 + rate limit |

---

# 7. P1 (30일) + P2 (차후)

## <span class="pill pill-p1">P1</span> High Priority

- **LLM 한도**: Gemini 무료 한도가 작아 실패 시 키워드 방식으로 떨어짐 → 유료 전환 검토 (정책: GPT·Gemini·Claude 중 1개)
- **AI 요약 검증**: 요약 속 숫자를 원본과 자동 대조하는 장치
- **테스트**: 화면 코드(.jsx)가 린트·테스트 대상에서 빠져 있음 → 브라우저 자동 검사를 CI에 편입
- **관측성**: 수집 실패·갱신 중단 발생 시 알림

## <span class="pill pill-p2">P2</span> 차후

- **법무**: RSS 저작권 약관 점검, 뉴스 출처 명시 강화
- **사용성**: 주간 리포트, 멀티유저, i18n
- **모델**: 검증 합격 이후 — 변화율·분포 예측, 설명 가능성

---

<!-- _class: title -->

# Thank you 🙇

<br>

## Q & A

<br>

| | |
|---|---|
| 현재 지표 | 메모리 3사 주가지수 **751.1 pt** (2026-09-21 · 4주 +14.4%) |
| 예측 검증 | **불합격** — 모델 33.8% vs 단순 기준선 15.7% (231회 워크포워드) |
| 수집 현황 | 14개 신호 중 정상 12 · 갱신 중단 1 · 제외 1 · 거시 6종 모두 정상 |
| AI 요약 | Gemini 무료 티어 · 사실만 · "검증되지 않은 해석" 표시 |
| 자동화 | GitHub Actions 매주 화 06:00 KST + 앱 버튼 (비용 0원) |
| GitHub commits | **127개** |
| 현재 버전 | **v2.5** (2026-09-26) |

<br>

<span class="small">배포: https://sixsense-eta.vercel.app</span>
