...
footer: 'KAIST CAIO 10기 6조 · v2.6 · 2026-09-26'
style: |
  section {
...
<span class="small">KAIST CAIO 10기 6조 · v2.6 (2026-09-26 — 예측 검증 + 화면 정직화 + 수집기 복구 + 재현성 + 불합격 예측의 참고 표시 + 최종 검증 반영)</span>
...
| 3 | **화면 구성** — v2.3 정직화 | 3분 |
| 4 | **Architecture & Pipeline** — 데이터 흐름 5단계 | 3분 |
...
| 영역 | 결과 |
|---|---|
| 🎯 **자동 수집** | 매주 화 06:00 무료 자동 실행 (GitHub Actions) · 이번 주 14개 신호 중 **정상 13 · 제외 1** (A-2 — 자동 데이터 검사가 날짜 형식 오류 적발, 화면에 사유 표시) |
| 🔍 **예측 검증** | AI 모델 오차 <span class="neg">34.3%</span> vs 단순 기준선(지난주 값 유지) <span class="pos">15.7%</span> → **불합격** — 예측값은 "❌ 불합격 · 참고용" 표시와 불합격 이유 설명을 붙여 표시 |
...
│                              + 지금부터 1~7주 예측 (검증과 같은 모델)    │
│  ④ build_insight.py      →  Gemini 사실 요약 + 불합격 이유(숫자 대조)   │
...
| **v2.6** | **2026-09-26** | **재발 방지 장치 — 자동 시험 55개·돌연변이 시험·데이터 의미 검사·커밋/주간 작업 관문·작업 규칙** |

<br>

**Git history**: 139 commits · 문서: [prd.md](../../prd.md) · [plan](../01-plan/features/sixsense.plan.md) · [design](../02-design/features/sixsense.design.md) · [CHANGELOG](../../CHANGELOG.md)
...
| 이전 표시 | 실제 | v2.3 |
|---|---|---|
| "현재 계약가 · DDR5 8Gb · $7.47/GB" | 메모리 3사 주가지수 ÷ 100 | **메모리 3사 주가지수 751.1 pt** + 대용 지표 고지 |
...
| "수집 안정성 최근 8주 100% 성공" | 고정 문장 | 실제 상태: 정상 13 · 제외 1 |
...
| | A-4 | 반도체 재고지수 | KOSIS 광업제조업동향조사 · 반도체 제조업(C261) 생산자제품 재고지수, 2020=100 |
...
<span class="small">화면 미표시(검증에는 사용): B-2 대만 뉴스 · B-3 HN · B-4 지정학(GPR) · 화면·검증 모두 제외: A-2 빅테크 CapEx(날짜 형식 오류·수집기 없음)</span>
...
| 신호 | 상태 | 사유 (자동 판정) |
|---|---|---|
| A-2 빅테크 CapEx | ⛔ 제외 | 자동 데이터 검사: 57주 모두 월요일이 아닌 날짜 · 2026-07-12 이후 수집기 없음 (v2.6) |
| A-3 관세청 수출 | ✅ 정상 | 2026-08 통계까지 반영 (v2.3.2 — 아래 정정 참고) |
| A-4 반도체 재고지수 | ✅ 정상 | 2026-07 통계 (v2.5.1 에서 올바른 표로 복구 — 아래 정정 2) |
...
- 한국 정부 API(A-3·A-4)는 GitHub 서버(미국 IP)에서 **간헐 차단** (9/24~25 4회는 정상, 9/26 은 둘 다 차단) → 실패하면 기존 값 유지, 한국에서 로컬 수집으로 보충
- **A-4 복구(v2.5.1)**: KOSIS 표 DT_1F02001 · 반도체 제조업(C261) 생산자제품 재고지수(원지수)로 수집 — 달마다 1행, 응답이 이 지표가 아니면 저장하지 않음
- **정정 2**: v2.3.2 에서 A-4 를 "정상(발표 지연)"이라 했으나 사실이 아님 — 조회 주소가 반도체가 없는 다른 표를 가리켜, 달마다 330행 중 임의 한 행이 저장되고 있었음 (2026-05-17 도입 이후). v2.4.1 에서 화면·예측에서 제외
...
- **데이터 교정**: 날짜를 월요일로 통일 · 미래 값으로 과거를 채우지 않음 · 월간 통계는 **발표 지연 7주** 반영
- **비교 기준**: **단순 기준선** = "지난주 값이 그대로 유지된다"
- **합격 기준**: 모델 평균 오차 < 기준선 평균 오차 **그리고** 4주 예측의 Diebold–Mariano 검정(겹치는 예측 보정) p < 0.05
...
|---|---|---|---|
| 평균 오차 (MAPE) | <span class="neg">34.3%</span> | 21.9% | <span class="pos">15.7%</span> |
| 기준선을 이긴 비율 | 15.6% | 54.5% (p = 0.736, 우연과 구분 안 됨) | — |
| 오르내림 적중률 | 32.5% | 70.1% | "항상 오른다" 찍기 70.6% |
...
- p값 정정(v2.5): 예전 방식(1~7주 예측을 독립으로 세는 부호검정)은 겹치는 예측 때문에 p 가 작게 나왔음 — 개선 시도 0.146 → 겹침 보정 후 0.736
...
> 2주 동안 확인된 실수 약 29건의 반복 원인을 **사람의 주의력이 아니라 자동 장치**로 막는다

| 장치 | 무엇을 막나 | 강제 방식 |
|---|---|---|
| **자동 시험 55개** (`backend/tests`) | 과거 사고 재발 (사고 1건당 시험 1개 이상) | 커밋 관문 · 주간 작업 시작 전 |
| **돌연변이 시험** | 쓸모없는 시험 — 과거 버그 14개를 되살려 **14개 모두 적발** 확인 | verify.sh |
| **데이터 의미 검사** (21개 신호) | 엉뚱한 표·단위·날짜 (예: A-4 492만) → **자동 제외** + 사유 | 화면·AI 요약·예측 검증 공통 |
| **커밋·주간 작업 관문** | 검증 없이 "됐다" · 잘못된 코드로 만든 데이터 배포 | `scripts/verify.sh` 실패 시 차단 |
| **작업 규칙** (`CLAUDE.md`) | 원천 확인 없이 단정 · 부분 수정 · 가정 미확인 | 매 대화 자동 적용 |

<span class="small">장치 가동 첫날 실제 적발: A-2 날짜 형식 오류(자동 제외), 뉴스 10건이 조용히 0건이 되던 결함(v2.5 수정의 부작용)</span>
---
...
| Step | 화면 | 강조 포인트 |
|---|---|---|
| 1 | **S-001 §01 지표 스냅샷** | 주가지수 751.1 pt · 대용 지표 고지 · 실측 4주 +14.4% · 예측 검증 "불합격" |
...
| 5 | **S-014 수집 현황** | 정상 13 · 제외 1 (A-2, 자동 데이터 검사) · 사유 자동 표시 |
...
| <span class="pill pill-p0">P0</span> | **멈춘 수집** | 제외 1개 (A-2 수집기 없음·날짜 형식 오류) — 거시 6종 · A-1 · A-7 복구(v2.3.1), A-3 조회 기간 고정 해제(v2.3.2) | A-2 분기 CapEx 수집기 |
...
| | |
|---|---|
| 현재 지표 | 메모리 3사 주가지수 **751.1 pt** (2026-09-21 · 4주 +14.4%) |
| 예측 검증 | **불합격** — 모델 34.3% vs 단순 기준선 15.7% (231회 워크포워드) |
| 수집 현황 | 14개 신호 중 정상 13 · 제외 1 · 거시 6종 모두 정상 |
| AI 요약 | Gemini 무료 티어 · 사실만 · "검증되지 않은 해석" 표시 |
| 자동화 | GitHub Actions 매주 화 06:00 KST + 앱 버튼 (비용 0원) |
| GitHub commits | **139개** |
| 현재 버전 | **v2.6** (2026-09-26) |
