...
| #13 | §01 인사이트 가독성 미세조정 (2026-05-19) | 사용자 보고 (a) **insight-main 좌우 분할이 화면 너비 1400px 이하에서 위·아래로 떨어짐** → breakpoint **1400px → 800px** 로 대폭 낮춰 일반 노트북/데스크탑 환경에서 항상 좌우 분할(3:2) 유지, (b) 카드 안 본문(.insight-body) 폰트 **14px → 11.5px** (1.5pt ↓) + line-height 1.7→1.65 로 컴팩트화 (모달 본문 .insight-modal-summary 는 14.5px 유지 — 전체 화면 표시용이라 큰 폰트 유지) | styles.css `.insight-main` 미디어 쿼리 + `.insight-body` font-size · `.gitignore` 에 `.claude/worktrees/` 추가 |
